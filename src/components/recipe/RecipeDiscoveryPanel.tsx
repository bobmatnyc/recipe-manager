...
export function RecipeDiscoveryPanel() {
  ...
  const handleDiscover = async () => {
    ...
    setProgress({ step: 'searching', currentIndex: 0, totalItems: 0, message: 'Searching for recipes...' });
    ...
    try {
      ...
      setProgress({ step: 'complete', currentIndex: result.stats.saved, totalItems: result.stats.searched, message: 'Discovery complete!' });
  ...
  const handleDiscoverFromUrl = async (url: string) => {
    ...
    setDiscoveryResults(null);
    setDiscoveredRecipes([]);
    setProgress({ step: 'validating', currentIndex: 1, totalItems: 1, message: 'Extracting recipe from URL...' });
    ...
    try {
      ...
      if (result.success && result.recipe) {
        ...
        setProgress({ step: 'complete', currentIndex: 1, totalItems: 1, message: 'Recipe extracted successfully!' });
        ...
        <CardContent className="space-y-4">
          ...
          <details className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-3">
              ...
              <div>
                <Label htmlFor="minConfidence">Min Confidence ({(minConfidence * 100).toFixed(0)}%)</Label>
                <Input
                  id="minConfidence"
                  type="range"
                  min="0"
          ...
          <CardContent className="pt-6">
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2 text-sm">
                <div className={`flex items-center gap-1 ${progress.step === 'searching' ? 'text-primary' : 'text-muted-foreground'}`}>
                  <Search className="w-4 h-4" />
                  <span>Search</span>
                </div>
                <div className="flex-1 h-px bg-border" />
                <div className={`flex items-center gap-1 ${progress.step === 'validating' ? 'text-primary' : 'text-muted-foreground'}`}>
                  <CheckCircle2 className="w-4 h-4" />
                  <span>Validate</span>
                </div>
                <div className="flex-1 h-px bg-border" />
                <div className={`flex items-center gap-1 ${progress.step === 'tagging' ? 'text-primary' : 'text-muted-foreground'}`}>
                  <Tag className="w-4 h-4" />
                  <span>Tag</span>
                </div>
                <div className="flex-1 h-px bg-border" />
                <div className={`flex items-center gap-1 ${progress.step === 'embedding' ? 'text-primary' : 'text-muted-foreground'}`}>
                  <TrendingUp className="w-4 h-4" />
                  <span>Embed</span>
                </div>
                <div className="flex-1 h-px bg-border" />
                <div className={`flex items-center gap-1 ${progress.step === 'saving' ? 'text-primary' : 'text-muted-foreground'}`}>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-primary">{discoveryResults.stats.searched}</div>
                <div className="text-xs text-muted-foreground">Searched</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-green-600">{discoveryResults.stats.validated}</div>
                <div className="text-xs text-muted-foreground">Validated</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600">{discoveryResults.stats.saved}</div>
                <div className="text-xs text-muted-foreground">Saved</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-yellow-600">{discoveryResults.stats.skipped}</div>
                <div className="text-xs text-muted-foreground">Skipped</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-red-600">{discoveryResults.stats.failed}</div>
                      ...
                      {discoveryResults.errors.slice(0, 5).map((error, idx) => (
                        <li key={idx} className="flex items-start gap-2">
                          ...
                          <span>
                            ...
                            {error.url !== 'pipeline' && (
                              <a href={error.url} target="_blank" rel="noopener noreferrer" className="ml-1 underline">
                                (link)
                              </a>
                            )}
                          </span>
                ...
                <CardContent className="space-y-3">
                  ...
                  {parseTags(recipe.tags).length > 0 && (
                    <div className="flex flex-wrap gap-1">
                      {parseTags(recipe.tags).slice(0, 4).map((tag, idx) => (
