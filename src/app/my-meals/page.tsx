...
export default async function MyMealsPage({ searchParams }: PageProps) {
  const { userId } = await auth();

  if (!userId) {
    redirect('/sign-in?redirect=/my-meals');
  }

  const params = await searchParams;
  const mealType = params.type || 'all';

  // Fetch user's meals (private and public)
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-jk-charcoal/70 font-body">
            Manage your personal meal plans and shopping lists
          </p>
        </div>
        <Link href="/meals/new">
      ...
      {/* Filter */}
      <div className="mb-6">
        <form action="/my-meals" method="get">
      ...
      {/* Meals list */}
      <MealsList initialMeals={initialMeals} mealType={mealType} isPublicView={false} />
    </div>
  );
}
...
export const metadata = {
  ...
  description: 'Manage your personal meal plans and shopping lists',
};
