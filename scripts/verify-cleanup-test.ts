...
async function verifyCleanup() {
  ...
  const recipe = await db
    .select()
    .from(recipes)
    .where(eq(recipes.id, '2758ce47-14a2-4b62-846d-ed1d713460b0'))
    .limit(1);
  ...
  if (recipe[0]) {
    ...
    const hasAmount = (ing: string) =>
      /^[\d½¼¾⅓⅔⅛⅜⅝⅞]|^(a |an |one |two |three |some |few |several )/i.test(ing.trim());
    const withAmounts = ingredients.filter((ing: string) => hasAmount(ing)).length;

    console.log(`\n📊 Stats:`);
    console.log(`   Total ingredients: ${ingredients.length}`);
    console.log(
      `   With amounts: ${withAmounts} (${((withAmounts / ingredients.length) * 100).toFixed(0)}%)`
    );
  }

  // Check another complex recipe (Mole Negro)
  const moleRecipe = await db
    .select()
    .from(recipes)
    .where(eq(recipes.id, '5a15ab37-1b32-40aa-ba70-e341dd8baeee'))
    .limit(1);
