...
async function generateTomatoLogos() {
  ...
  await sharp(svgBuffer).resize(1024, 1024).png().toFile(join(PUBLIC_DIR, 'ai-tomato-logo.png'));
  console.log('✅ ai-tomato-logo.png created');

  // Generate favicon (32x32)
  console.log('Creating favicon (32x32)...');
  await sharp(svgBuffer).resize(32, 32).png().toFile(join(PUBLIC_DIR, 'icon.png'));
  ...
  await sharp(svgBuffer).resize(180, 180).png().toFile(join(PUBLIC_DIR, 'apple-touch-icon.png'));
