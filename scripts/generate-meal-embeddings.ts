...
/**
 * Generate embeddings for ALL meals in the database
 *
 * Production-ready script with checkpoint/resume, detailed logging, and safety features
 * Processes meals in batches to avoid overwhelming the API
 *
 * Features:
 * - Checkpoint/resume capability every 50 meals
 ...
 * - Fetches recipe names for richer embeddings
 *
 * Usage:
 *   npx tsx scripts/generate-meal-embeddings.ts                    # Dry run (default)
 *   npx tsx scripts/generate-meal-embeddings.ts --execute          # Execute (with confirmation)
 *   npx tsx scripts/generate-meal-embeddings.ts --limit=10         # Test on 10 meals
 *   npx tsx scripts/generate-meal-embeddings.ts --resume           # Resume from last checkpoint
 *   npx tsx scripts/generate-meal-embeddings.ts --batch-size=10    # Custom batch size
 *   npx tsx scripts/generate-meal-embeddings.ts --delay=1000       # Custom delay (ms)
 */
...
const CHECKPOINT_INTERVAL = 50; // Save checkpoint every 50 meals
const MAX_RETRIES = 3;

// Types
interface Checkpoint {
  timestamp: string;
  processedMealIds: string[];
...
async function saveCheckpoint(checkpoint: Checkpoint, tmpDir: string): Promise<string> {
  const checkpointPath = path.join(tmpDir, 'meal-embedding-generation-checkpoint.json');
...
async function appendErrorLog(error: FailedMeal, tmpDir: string): Promise<void> {
  const logPath = path.join(tmpDir, 'meal-embedding-errors.log');
  const timestamp = new Date().toISOString();
  const logLine = `[${timestamp}] Meal: ${error.mealName} (${error.mealId}) | Attempts: ${error.attempts} | Error: ${error.error}\n`;
...
async function getRecipeNamesForMeal(mealId: string): Promise<string[]> {
  try {
    ...
    console.warn(`Failed to fetch recipe names for meal ${mealId}:`, error);
    return [];
  }
}
...
async function generateEmbeddingWithRetry(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      // Fetch recipe names for richer embedding
      const recipeNames = await getRecipeNamesForMeal(meal.id);
      ...
      await saveMealEmbedding(meal.id, result.embedding, result.embeddingText, result.modelName);
...
async function main() {
  ...
  log('PRODUCTION-READY EMBEDDING GENERATION FOR ALL MEALS', colors.bright);
  log(`${'='.repeat(80)}\n`, colors.bright);
  ...
  let processedMealIds = new Set<string>();

  if (RESUME) {
    checkpoint = await loadCheckpoint(tmpDir);
    if (checkpoint) {
      log(
        `\n📂 Resuming from checkpoint (${checkpoint.processedMealIds.length} meals already processed)`,
        colors.cyan
      );
      processedMealIds = new Set(checkpoint.processedMealIds);
    } else {
  ...
  const totalMeals = await db.execute(sql`SELECT COUNT(*) as count FROM meals`);
  const mealCount = Number(totalMeals.rows[0]?.count || 0);
  const embeddingCount = await countMealEmbeddings();
  let missing = mealCount - embeddingCount;

  // Adjust for already processed in this session
  if (checkpoint) {
    missing = Math.max(0, missing - processedMealIds.size);
  }
  ...
  log(`  Total meals: ${mealCount.toLocaleString()}`, colors.blue);
  log(
  ...
  if (checkpoint) {
    log(
      `  Already processed (this session): ${processedMealIds.size.toLocaleString()}`,
      colors.cyan
    );
  }

  if (targetCount === 0) {
    log('\n✅ All meals already have embeddings!', colors.green);
    await clearCheckpoint(tmpDir);
    process.exit(0);
  }

  // Estimate time and cost
  const avgTimePerMeal = 3; // seconds (includes API call + delay)
  const estimatedSeconds = targetCount * avgTimePerMeal;
  ...
  log(`  Batch size: ${BATCH_SIZE} meals`, colors.blue);
  log(`  Delay between batches: ${DELAY_MS}ms`, colors.blue);
  log(`  Max retries: ${MAX_RETRIES} attempts`, colors.blue);
  log(`  Checkpoint interval: Every ${CHECKPOINT_INTERVAL} meals`, colors.blue);
  ...
  while (stats.processed < targetCount) {
    // Get next batch, excluding already processed meals
    let query = sql`
      SELECT m.*
      FROM meals m
    ...
    // Exclude already processed meals if resuming
    if (processedMealIds.size > 0) {
      const idsArray = Array.from(processedMealIds);
      query = sql`
        SELECT m.*
        FROM meals m
        LEFT JOIN meals_embeddings e ON m.id = e.meal_id
    ...
    const meals = batch.rows as any[];

    if (meals.length === 0) {
      log('\n✅ No more meals to process', colors.green);
      break;
    }

    const batchNum = Math.floor(stats.processed / BATCH_SIZE) + 1;
    log(`\n[Batch ${batchNum}] Processing ${meals.length} meals...`, colors.cyan);
    ...
    for (let i = 0; i < meals.length; i++) {
      ...
      log(`\n[${overall}/${targetCount}] (${percentage}%) ${meal.name}`, colors.blue);
      log(`  ID: ${meal.id}`, colors.blue);

      if (DRY_RUN) {
        log(`  ✓ Would generate embedding (dry run)`, colors.green);
        stats.success++;
      } else {
        const result = await generateEmbeddingWithRetry(meal, MAX_RETRIES, tmpDir);
        ...
        // Delay to avoid rate limits (except on last meal)
        if (i < meals.length - 1 && overall < targetCount) {
      ...
      if (stats.processed % CHECKPOINT_INTERVAL === 0 && !DRY_RUN) {
        const checkpointData: Checkpoint = {
          ...
          processedMealIds: Array.from(processedMealIds),
          stats,
        };
    ...
    const rate = stats.processed / elapsed; // meals per second
    ...
    log(`  Rate: ${(rate * 60).toFixed(1)} meals/min`, colors.blue);
  ...
  const finalCount = await countMealEmbeddings();
  const totalMealsResult = await db.execute(sql`SELECT COUNT(*) as count FROM meals`);
  const totalMealsCount = Number(totalMealsResult.rows[0]?.count || 0);
  const coverage = ((finalCount / totalMealsCount) * 100).toFixed(1);
  ...
  log(`  Average rate: ${((stats.processed / totalTime) * 60).toFixed(1)} meals/min`, colors.blue);

  log(`\n📈 Database Coverage:`, colors.cyan);
  log(`  Total meals: ${totalMealsCount.toLocaleString()}`, colors.blue);
  log(`  With embeddings: ${finalCount.toLocaleString()} (${coverage}%)`, colors.green);

  if (stats.failed > 0) {
    log(`\n⚠️  Failed Meals (${stats.failed}):`, colors.yellow);
    const displayCount = Math.min(10, failedMeals.length);
    for (let i = 0; i < displayCount; i++) {
      const failed = failedMeals[i];
      log(`  ${i + 1}. ${failed.mealName}`, colors.red);
      log(`     ID: ${failed.mealId}`, colors.red);
  ...
  // Save final report
  if (!DRY_RUN) {
    const reportPath = path.join(tmpDir, `meal-embedding-generation-report-${getTimestamp()}.json`);
    await fs.writeFile(
      reportPath,
      JSON.stringify(
        {
