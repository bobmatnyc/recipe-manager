 ...
 * Chef Import Script
 *
 * Imports celebrity chef profiles into the database.
 * Each chef includes bio, specialties, and social media links.
 */
...
const CHEFS: ChefData[] = [
  ...
  },
  {
    ...
    display_name: 'J. Kenji López-Alt',
...
async function importChefs() {
  console.log('Starting chef import...\n');
  ...
  for (const chefData of CHEFS) {
    try {
      ...
      const existing = await db.select().from(chefs).where(eq(chefs.slug, chefData.slug)).limit(1);

      const chefRecord = {
        name: chefData.name,
        display_name: chefData.display_name || chefData.name,
        slug: chefData.slug,
        bio: chefData.bio,
        profile_image_url: `/chefs/avatars/${chefData.slug}.jpg`,
        specialties: chefData.specialties,
        website: chefData.website || null,
        social_links: {
          instagram: chefData.instagram,
          twitter: chefData.twitter,
          youtube: chefData.youtube,
        },
        is_verified: true,
        is_active: true,
        updated_at: new Date(),
      ...
      if (existing.length > 0) {
        ...
        await db.update(chefs).set(chefRecord).where(eq(chefs.slug, chefData.slug));

        console.log(`✓ Updated: ${chefData.name}`);
        updated++;
      } else {
        // Insert new chef
        await db.insert(chefs).values(chefRecord);
