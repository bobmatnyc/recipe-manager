...
async function testAccessControl() {
  ...
  const ownRecipe = await db.select().from(recipes).where(eq(recipes.id, user1Recipe.id)).limit(1);
  ...
  const sysRecipe = await db.select().from(recipes).where(eq(recipes.id, systemRecipe.id)).limit(1);
  ...
  console.log(
    '   - "You do not have permission to delete this recipe" - for other users\' recipes'
  );
  ...
  const publicRecipes = await db.select().from(recipes).where(eq(recipes.isPublic, true));

  console.log(`✅ Found ${publicRecipes.length} public recipes (viewable by all)`);

  const privateRecipes = await db.select().from(recipes).where(eq(recipes.isPublic, false));
